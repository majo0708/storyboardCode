...
{
    public class ScreenCoverAlt : StoryboardObjectGenerator
    {
        [Configurable]
        ...
        public void Spiral(double beat, float rotation, Color4 color)
        {
            var coverLayer = GetLayer("Cover");
            
            int i = 0;
            Vector2 startPos = onScreen1;
            while (StartTime + i*beat < EndTime)
            {
                var spiralSprite = coverLayer.CreateSprite(SpritePath, OsbOrigin.Centre);
                spiralSprite.Scale(StartTime + i*beat, SpriteScale);
                spiralSprite.Color(StartTime + i*beat, color);
                spiralSprite.Rotate(OsbEasing.None, StartTime + i*beat, StartTime + (i+2)*beat, (rotation + i*30) * Math.PI / 180 + Math.PI/2, (rotation + i*30) * Math.PI / 180 + 3*Math.PI/4);
                startPos = onScreen1;

                for (int j = 0; j < 60; j++)
                {
                    Vector2 nextPos = new Vector2(startPos.X + 12*(float)Math.Cos((rotation + i*30) / 180 * Math.PI + 2*Math.PI*j/480), startPos.Y + 12*(float)Math.Sin((rotation + i*30) / 180 * Math.PI + 2*Math.PI*j/480));
                    spiralSprite.Move(StartTime + i*beat + beat*j/30, StartTime + i*beat + beat*(j+1)/30, startPos, nextPos);
                    startPos = nextPos;
                }
        ...
        public override void Generate()
        {
		    ...
		    Spiral(beat, Rotation, Color);
            Spiral(beat, Rotation + 45, Color);
            Spiral(beat, Rotation + 90, Color);
            Spiral(beat, Rotation + 135, Color);
            Spiral(beat, Rotation + 180, Color);
            Spiral(beat, Rotation + 225, Color);
            Spiral(beat, Rotation + 270, Color);
            Spiral(beat, Rotation + 315, Color);
        }
    }
}
