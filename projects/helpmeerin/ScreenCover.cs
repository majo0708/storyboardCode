...
{
    public class ScreenCover : StoryboardObjectGenerator
    {
        [Configurable]
        ...
        public void Spiral(double beat, float rotation, Color4 color)
        {
            ...
            while (StartTime + i*beat < EndTime)
            {
                ...
                for (int j = 0; j < 240; j++)
                {
                    Vector2 nextPos = new Vector2(startPos.X + 3*(float)Math.Cos((rotation + i*30) / 180 * Math.PI + 2*Math.PI*j/480), startPos.Y + 3*(float)Math.Sin((rotation + i*30) / 180 * Math.PI + 2*Math.PI*j/480));
        ...
        public override void Generate()
        {
		    ...
		    Spiral(beat, Rotation, Color4.Red);
            Spiral(beat, Rotation + 45, Color4.Orange);
            Spiral(beat, Rotation + 90, Color4.Yellow);
            Spiral(beat, Rotation + 135, Color4.Green);
            Spiral(beat, Rotation + 180, Color4.SkyBlue);
            Spiral(beat, Rotation + 225, Color4.Blue);
            Spiral(beat, Rotation + 270, Color4.Indigo);
            Spiral(beat, Rotation + 315, Color4.Violet);
        }
    }
}
